...
/* openela 4.14 / some OEM kernels renamed type_val_to_struct
 * to type_val_to_struct_array (still a pointer array) */
#if defined(KSU_COMPAT_USE_TYPE_VAL_TO_STRUCT_ARRAY)
#define KSU_TYPE_VAL_TO_STRUCT_FIELD(db_ptr) (db_ptr)->type_val_to_struct_array
#else
#define KSU_TYPE_VAL_TO_STRUCT_FIELD(db_ptr) (db_ptr)->type_val_to_struct
#endif
...
static bool add_type(struct policydb *db, const char *type_name, bool attr)
{
    ...
    struct type_datum **new_type_val_to_struct = ksu_kvrealloc(
        KSU_TYPE_VAL_TO_STRUCT_FIELD(db), sizeof(*KSU_TYPE_VAL_TO_STRUCT_FIELD(db)) * value, sizeof(*KSU_TYPE_VAL_TO_STRUCT_FIELD(db)) * (value - 1));
    ...
    KSU_TYPE_VAL_TO_STRUCT_FIELD(db) = new_type_val_to_struct;
    KSU_TYPE_VAL_TO_STRUCT_FIELD(db)[value - 1] = type;
   ...
   */
    ...
    struct type_datum **new_type_val_to_struct =
        krealloc(KSU_TYPE_VAL_TO_STRUCT_FIELD(db), sizeof(*KSU_TYPE_VAL_TO_STRUCT_FIELD(db)) * db->p_types.nprim, GFP_ATOMIC);
...
static void free_type_datum_partially(struct policydb *db)
{
    ...
    if (KSU_TYPE_VAL_TO_STRUCT_FIELD(db)) {
        kvfree(KSU_TYPE_VAL_TO_STRUCT_FIELD(db));
    }
...
static int copy_type_datum_partially(struct policydb *new_db, struct policydb *old_db)
{
    ...
    KSU_TYPE_VAL_TO_STRUCT_FIELD(new_db) = NULL;
    ...
    new_type_val_to_struct = kvcalloc(sz, sizeof(*KSU_TYPE_VAL_TO_STRUCT_FIELD(new_db)), GFP_KERNEL);
    if (!new_type_val_to_struct) {
        goto out;
    }
    KSU_TYPE_VAL_TO_STRUCT_FIELD(new_db) = new_type_val_to_struct;
    memcpy(KSU_TYPE_VAL_TO_STRUCT_FIELD(new_db), old_db->type_val_to_struct, sz * sizeof(*KSU_TYPE_VAL_TO_STRUCT_FIELD(new_db)));
